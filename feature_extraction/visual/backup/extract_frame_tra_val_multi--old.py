...
class FrameDataset(data.Dataset):
    def __init__(self, vid, face_dir, transform=None):
        ...
        # Ensure only two frames are selected if less than two are available
        if len(self.frames) < 2:
            self.frames = random.sample(self.frames, len(self.frames))  # Randomly sample all available frames
        else:
            self.frames = random.sample(self.frames, 2)  # Randomly sample 2 frames
...
def visual_extraction():
    ...
    for video in tqdm.tqdm(video_name):
        if 'mp4' in video:
            ...
            if not os.path.exists(f'/home/ccip/data/wangyuanxiang/xyy/reproduce/Track2/English/video_multi_frame_English_{sample_rate}'):
                os.mkdir(f'/home/ccip/data/wangyuanxiang/xyy/reproduce/Track2/English/video_multi_frame_English_{sample_rate}')
            frame_extract(video_path, f'/home/ccip/data/wangyuanxiang/xyy/reproduce/Track2/English/video_multi_frame_English_{sample_rate}', sample_rate=sample_rate)
    ...
    english_frame_dir = r'/home/ccip/data/wangyuanxiang/xyy/reproduce/Track2/English/video_multi_frame_English_10'
    save_dir = r'/home/ccip/data/wangyuanxiang/xyy/reproduce/Track2/English/features/local_resnet50_multi_frame_10'
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
