...
class MultimodalDataset(BaseDataset):
    ...
    def __init__(self, opt, set_name):
        ...
        # xyy: csv
        # tra and val
        target_file = os.path.join(config['target_root'], f"transcription.csv")
        # tra or val
        # target_file = os.path.join(config['target_root'], f"{set_name}_transcription.csv")
        ...
        with open(target_file, 'r', encoding='utf-8') as f:
            reader = csv.reader(f)  # 使用csv.reader来读取CSV文件
            for row in reader:
                if row[0] == "Subtitle":
                # if row[0] == "Subtitle" or set_name=="Validation" or set_name=="Testing":
                    continue
                # file_name = row[0][:-4]  # 假设文件名在第一个字段，且后缀长度为4
                # xyy: read file_name
                file_name = row[1][:]
                # print('#file_name', file_name)
                # print('#row', row)  # 使用row代替cut，因为row已经是分割后的列表
                if set_name!="Testing" and set_name in file_name:
                    emo_label = emotion_dict.get(row[2], "Unknown")  # 使用get方法，如果键不存在则返回"Unknown"
                    int_label = intent_dict.get(row[3], "Unknown")  # 同上
                    # print("##[emo_label, int_label]",[emo_label, int_label])
                    self.sample.append([file_name, emo_label, int_label])
                    # print("##self.sample", self.sample[0][0])
    ...
    def __getitem__(self, index):
        ...
        A_feat_path = os.path.join(self.all_A_path, self.sample[index][0][:-4] + '.npy')
        V_feat_path = os.path.join(self.all_V_path, self.sample[index][0][:-4] + '.npy')
        L_feat_path = os.path.join(self.all_L_path, self.sample[index][0][:-4] + '.npy')
