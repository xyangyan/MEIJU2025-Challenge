        ...
        with open(target_file, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                # if row[0] == "Subtitle" or set_name=="Validation" or set_name=="Testing":
                    continue
                file_name = row[1][:]
                # file_name = row[0][:-4]  # 假设文件名在第一列，且文件名后缀长度为4
                self.sample.append(file_name)

    def __getitem__(self, index):
        int2name = self.sample[index]

        A_feat_path = os.path.join(self.all_A_path, self.sample[index][:-4] + '.npy')
        V_feat_path = os.path.join(self.all_V_path, self.sample[index][:-4] + '.npy')
        L_feat_path = os.path.join(self.all_L_path, self.sample[index][:-4] + '.npy')
