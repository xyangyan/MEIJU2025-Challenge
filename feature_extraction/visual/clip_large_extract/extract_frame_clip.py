...
def func_read_frames(face_dir, vid):
    # 构造图片文件夹路径
    img_dir = os.path.join(face_dir, vid)  # 每个视频对应的子文件夹
    # 获取文件夹中的所有图片文件
    img_files = os.listdir(img_dir)
    
    frames = []
    for img_file in sorted(img_files):  # 确保按照文件名排序
        img_path = os.path.join(img_dir, img_file)
        if img_file.endswith(('.jpg', '.jpeg', '.png')):  # 只读取图片文件
            img = cv2.imread(img_path)  # 读取图片
            if img is not None:
                frames.append(img)  # 将读取的图片添加到帧列表中
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='Track2_English', help='input dataset')
